...
AI Daily Assistant 是基于 Hermes Agent 构建的智能日报系统，自动采集 AI/Agent 领域技术文章，经过 AI 筛选、智囊团多视角分析、标准化排版后，推送至飞书文档和微信。
...
| Feishu Card 推送 | 飞书卡片通知 | `send_feishu_card.py` |
| Feishu IM Dashboard | 飞书 IM 面板（不含微信对接） | 飞书 IM API |
| WeChat 推送 | 微信独立通道（非 IM 模块） | `send_message` → weixin gateway |
